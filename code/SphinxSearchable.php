		...
		foreach($this->sphinxFields($this->owner->class) as $name => $info) {
			...
			switch ($type) {
				...
				case 'HTMLText':
					...
					if($sortable && $db instanceof MSSQLDatabase || $db instanceof MSSQLAzureDatabase) {
						$select["_packed_$name"] = "(ascii(substring({$bt}$class{$bt}.{$bt}$name{$bt},1,1)) * 16777216) | (ascii(substring({$bt}$class{$bt}.{$bt}$name{$bt},2,1)) * 65536) | (ascii(substring({$bt}$class{$bt}.{$bt}$name{$bt},3,1)) * 256) | ascii(substring({$bt}$class{$bt}.{$bt}$name{$bt},4,1))";
						$attributes["_packed_$name"] = "uint";
					} elseif($sortable && !($db instanceof SQLite3Database || $db instanceof SQLitePDODatabase)) {
